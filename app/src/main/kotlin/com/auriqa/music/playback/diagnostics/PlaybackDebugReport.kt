...
object PlaybackDebugReportFormatter {
    fun format(
        ...
        appendLine()
        val reportTotalMs = failure.elapsedMs.takeIf { it > 0L }
            ?: events.maxOfOrNull(PlaybackDiagnosticEvent::elapsedMs)
            ?: 0L
        appendLine("Timing:")
        ...
        appendLine("totalMs=$reportTotalMs")
        appendLine("resolutionAttempts=${events.count { it is PlaybackDiagnosticEvent.ResolutionRequested }}")
        appendLine("playerResponseAttempts=${events.count { it is PlaybackDiagnosticEvent.PlayerResponseStart }}")
        appendLine("streamSelections=${events.count { it is PlaybackDiagnosticEvent.StreamSelected }}")
        appendLine("httpFailureCount=${events.count { it is PlaybackDiagnosticEvent.HttpStatus }}")
        appendLine()
        appendLine("Failure:")
